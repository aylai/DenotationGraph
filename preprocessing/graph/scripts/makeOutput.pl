...
#		}
        ...
        #print "add to queue $ax[2] $ntype[0] neg\n";
#		push(@queue, [ $ax[2], \@pid, \@pdep, \@pX, \@pY, \@ptype, $pn, 0, $ax[0] ]);
		...
		while ($#queue >= 0) {
            ...
            #print "generating sentences $q->[0]\n";
			...
			foreach my $k (keys %{$x}) {
				...
				if (not exists $graphvisit{$s}) {
                    #print "string not generated before\n";
					$graphvisit{$s} = {};
					...
					if ($q->[7] == -1) {
                        #print "add to queue $k $gntype{$id}[0] neg\n";
				...
				# otherwise, check if we've visited generated this node before with this particular caption and full string
                    ...
                    #print "not generated before with this caption/string\n";
                    #print "add to queue $k $gntype{$id}[0] pos\n";
					...
					if (exists $graphsent{$s}) {
                        ...
                        #print "found stored string $s $idStore\n";
                        #print "add to queue $graphstate{$s} $gptype{$idStore}[0] pos \n";
				...
				foreach my $l (keys %{$x->{$k}}) {
					...
					foreach (keys %{$x->{$k}->{$l}}) {
						...
						if (not exists $graphvisit{$t}) {
							...
							if ($q->[7] == -1) {
                                #print "add to queue $_ $gntype{$id}[0] neg\n";
						...
						# if this is an old node, but we haven't visited it with this caption/full string
                            ...
                            #print "old node, unvisited\n";
                            #print "add to queue $_ $gptype{$id}[0] pos\n";
							...
							if (exists $graphsent{$t}) {
                                ...
                                #print "paused caption $idStore\n";
                                #print "add to queue $graphstate{$t} $gptype{$idStore}[0] pos\n";
